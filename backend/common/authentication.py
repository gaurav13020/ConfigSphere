...
class JiraTokenUser:
    ...
    The auth-service is the source of truth for user records; the config
    server only needs the claims embedded in the token.
    """
...
class JiraJWTAuthentication(BaseAuthentication):
    ...
    Uses a shared JWT_SECRET_KEY — no inter-service call needed per request.
    Returns a JiraTokenUser (not a Django User model instance).
    """
    ...
    def authenticate(self, request):
        ...
        return (JiraTokenUser(payload), token)
